  ...
  try {
    ...
    if (response.type === 'opaqueredirect' || response.status === 0 || 
      ...
      if (location) {
        ...
        console.log('[Lock-in MediaFetcher] Following redirect to:', location, 'with credentials:', useCredentials);
        ...
        console.log('[Lock-in MediaFetcher] No location header (cross-origin redirect), trying with same-origin credentials');
        try {
    ...
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) {
        return { success: false, error: 'Authentication required. Please refresh the page and ensure you are logged in.', errorCode: 'AUTH_REQUIRED' };
      }
      return { 
        success: false, 
    ...
    if (!response.body) {
      return { success: false, error: 'No response body available', errorCode: 'NO_BODY' };
    }
    ...
    while (true) {
      ...
      if (done) {
        ...
        if (buffer.length > 0) {
          console.log('[Lock-in MediaFetcher] Sending final chunk:', chunkIndex, 'size:', buffer.length);
    ...
    if (message.includes('Failed to fetch') || message.includes('NetworkError') || message.includes('CORS')) {
      return { 
        success: false, 
        error: 'Could not fetch media. Your session may have expired - please refresh the page and ensure you are logged in.',
...
async function handleMediaFetchRequest(payload, sendChunk) {
  ...
  console.log('[Lock-in MediaFetcher] Handling fetch request:', { mediaUrl, jobId, requestId });
  
  return fetchMediaAsChunks(
    mediaUrl,
