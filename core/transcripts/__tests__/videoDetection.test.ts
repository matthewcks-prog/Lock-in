...
const buildHtmlDocument = (html: string): Document => {
...
describe('extractPanoptoInfo', () => {
  it('extracts from embed URL', () => {
    const info = extractPanoptoInfo(PANOPTO_EMBED_URL);
    expect(info).toEqual({
  ...
  it('extracts from viewer URL', () => {
    const info = extractPanoptoInfo(PANOPTO_VIEWER_URL);
    expect(info).toEqual({
      deliveryId: 'abc12345-1234-5678-9abc-def012345678',
      tenant: PANOPTO_TENANT,
    });
...
describe('detectPanoptoVideosFromIframes (basics)', () => {
  it('detects Panopto iframes', () => {
    const iframes = [
      buildIframe(PANOPTO_EMBED_URL, 'Week 1 Lecture'),
      buildIframe('https://youtube.com/embed/xyz', 'YouTube'),
    ];
  ...
  it('normalizes viewer URLs to embed URLs', () => {
    const iframes = [
      buildIframe('https://monash.au.panopto.com/Panopto/Pages/Viewer.aspx?id=abc123', 'Lecture'),
    ];
...
describe('detectPanoptoVideosFromIframes (dedupe)', () => {
  ...
  it('uses fallback title when none provided', () => {
    const iframes = [
      buildIframe('https://monash.au.panopto.com/Panopto/Pages/Embed.aspx?id=abc123'),
    ];
...
describe('detectVideosSync (panopto)', () => {
  it('detects Panopto iframes', () => {
    const result = detectVideosSync({
      pageUrl: 'https://learning.monash.edu/course/123',
      iframes: [
        buildIframe('https://monash.au.panopto.com/Panopto/Pages/Embed.aspx?id=abc123', 'Lecture'),
      ],
    });
...
describe('detectVideosSync (empty)', () => {
...
describe('detectVideosSync (html5)', () => {
...
describe('detectVideosSync (echo360)', () => {
...
describe('detectVideosSync (visibility)', () => {
