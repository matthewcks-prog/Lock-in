...
describe("NotesService", () => {
  ...
  describe("createNote", () => {
    ...
    it("should throw error when API client is null", async () => {
      ...
      await expect(service.createNote({ title: "Test", content: { version: "lexical_v1", editorState: null, plainText: "" } })).rejects.toThrow("Notes service requires an ApiClient instance");
    });
  });
  ...
  describe("getNote", () => {
    it("should get a note by ID", async () => {
      ...
      expect(mockApiClient.apiRequest).toHaveBeenCalledWith(
        "/api/notes/note-1",
        { method: "GET" }
      );
