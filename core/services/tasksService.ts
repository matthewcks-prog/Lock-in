  ...
  return async (params = {}): Promise<Task[]> => {
    try {
      ...
      const normalizedCourseCode =
        typeof courseCode === 'string' ? courseCode.trim() : (courseCode ?? undefined);
      const rawTasks = await apiClient.listTasks({
        ...(typeof normalizedCourseCode === 'string' && normalizedCourseCode.length > 0
...
function buildGetTask(apiClient: TasksApiClient, logger: Logger): TasksService['getTask'] {
  return async (taskId: string): Promise<Task> => {
    ...
    try {
      const raw = await apiClient.getTask(taskId.trim());
  ...
  return async (input: CreateTaskInput, options?: TaskRequestOptions): Promise<Task> => {
    ...
    try {
      ...
      const payload = {
        ...
        workflowStatus: input.workflowStatus ?? 'backlog',
...
function buildUpdateTask(apiClient: TasksApiClient, logger: Logger): TasksService['updateTask'] {
  return async (
    ...
    if (changes.title !== undefined && !isNonEmptyString(changes.title)) {
      throw new AppError('Task title cannot be empty', ErrorCodes.VALIDATION_ERROR);
    }

    try {
      const normalizedChanges =
        changes.title !== undefined ? { ...changes, title: changes.title.trim() } : changes;
      const payload = taskToPayload(normalizedChanges);
      const raw = await apiClient.updateTask(taskId.trim(), payload, {
        ...(options?.signal !== undefined ? { signal: options.signal } : {}),
...
function buildDeleteTask(apiClient: TasksApiClient, logger: Logger): TasksService['deleteTask'] {
  return async (taskId: string): Promise<void> => {
    ...
    try {
      await apiClient.deleteTask(taskId.trim());
...
function buildToggleCompleted(
  ...
  return async (taskId: string): Promise<Task> => {
    ...
    try {
      const raw = await apiClient.toggleTaskCompleted(taskId.trim());
      const task = normalizeTask(raw);
